...
int main(int argc, char *argv[])
{
    ...
    if(sigfillset(&blocked_signals) < 0) {
        ...
        memcpy(&s.sa_mask, &blocked_signals, sizeof(s.sa_mask));
    ...
    */
    ...
    if(( setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, (int *)&opt, sizeof(opt)) ) < 0 ){
      bail_out(EXIT_FAILURE, "Option set for socket failed");
    } 

    saddr.sin_addr.s_addr=INADDR_ANY;
    saddr.sin_port=htons((short)options.portno);
    ...
    connfd = accept(sockfd, (struct sockaddr*)&saddr, (socklen_t *)&saddr_size);
    if( connfd < 0 ){
      bail_out(EXIT_FAILURE, "accept client request");
    }
	...
	r = write(connfd, &buffer[0], WRITE_BYTES);
	if( r <= 0 ){
